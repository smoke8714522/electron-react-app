...
*   **Version Control & Grouping (Database Schema)**: Implemented in `main/schema.ts`.
    ...
    *   **UI**:
        ...
        *   Each master asset card (`AssetCard` within `LibraryView.tsx`) displays:
            *   A **Version Count badge** (e.g., `[Icon] 3`) in the top-right corner, showing the total number of assets in the group (`versionCount`). Displays `1` for standalone master assets. Uses `react-tooltip` for details.
            *   A **Shares label** in the main content area, always labeled "Shares:". If `versionCount > 1`, the displayed value is the `accumulatedShares` (total of master + versions). Otherwise, it displays the master's own `shares`. Values are formatted with `toLocaleString()`. The tooltip clarifies if the displayed value is accumulated (`Shares: 1,234 (Accumulated: Master + Versions)`).
...
*   **IPC Handlers**: Defined in `lib/main/main.ts` using `ipcMain.handle`:
    *   `get-assets`: Fetches asset metadata. Now filters to return **only master assets** (`master_id IS NULL`).
        ...
        *   `AssetSort`: `{ sortBy?: 'fileName' | 'year' | 'shares' | 'createdAt', sortOrder?: 'ASC' | 'DESC' }`. (Note: Sorting by `shares` currently uses the master asset's own `shares` value. Sorting by `accumulatedShares` or `versionCount` is not yet implemented in the `ORDER BY` clause).
...
*   **Main Library View Component**: `app/components/LibraryView.tsx` (Refactored)
    ...
    *   `AssetCard`: Displays thumbnail, key metadata, a version count badge (top-right), and a shares label. The label is always "Shares:", but the value shown is conditional (accumulated or master's own shares) based on the version count, clarified by a tooltip. Includes history button in footer.
...
*   **UI Layout**: `App.tsx` uses `flex flex-col h-screen w-screen` for full window layout. `LibraryView.tsx` uses `flex` for its two-pane layout, with the sidebar width controlled by state and the main content area managing its own scrolling. Tailwind CSS used throughout.
    *   **AssetCard**: Displays thumbnail, key metadata, a version count badge (top-right), and a shares/accumulated shares label. The version badge shows the total count of assets in the group. The shares label is always "Shares:", but the value shown is conditional (accumulated vs. master shares) based on the version count, clarified by a tooltip. Includes history button in footer.
